...
import configure.test.configurebuilds.activities.test101.Constraint101Activity
import configure.test.configurebuilds.application.model.ActivityItem
import configure.test.configurebuilds.databinding.ActivityConstraintListBinding


class ConstraintListActivity : AppCompatActivity() {

    lateinit var binding: ActivityConstraintListBinding

    private val list: MutableList<ActivityItem> = mutableListOf()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_constraint_list)
    ...
    private fun initializeUi() {
        ...
        if (list.isEmpty()) {
            list.add(ActivityItem(Constraint101Activity::class.java, "Constraint 101- preview"))
        }
